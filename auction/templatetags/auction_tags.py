...
def get_bid_rank(bid):
    """
    Returns the rank (1, 2, 3 or None) of a bid among unique bid amounts for its product.
    """
    if not bid or not bid.product_id:
        return None
        
    top_amounts = get_top_unique_bid_amounts(bid.product_id)
...
def get_user_product_rank(user, product):
    """
    Returns the rank (1, 2, 3 or None) of the user's highest bid for a product.
    """
    ...
    top_amounts = get_top_unique_bid_amounts(product_id)
